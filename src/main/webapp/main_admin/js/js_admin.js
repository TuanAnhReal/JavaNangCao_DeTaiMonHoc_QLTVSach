...
<!-- JAVASCRIPT LOGIC (MOCK DATA) -->
    ...
    <script type="module">
        ...
        let mockCategoryData = [
            { id: 'C001', name: 'Kinh Doanh', count: 45, status: 'Active' },
            { id: 'C002', name: 'Văn Học', count: 62, status: 'Active' },
            { id: 'C003', name: 'Kỹ Năng Sống', count: 30, status: 'Active' },
            { id: 'C004', name: 'Khoa Học', count: 15, status: 'Active' },
            { id: 'C005', name: 'Lịch Sử', count: 20, status: 'Active' },
            { id: 'C006', name: 'Thiếu Nhi', count: 10, status: 'Draft' },
            { id: 'C007', name: 'Tình Cảm', count: 35, status: 'Active' },
            // THÊM MỘT SỐ CATEGORY MOCK KHÔNG CÓ SÁCH ĐỂ TEST XOÁ
            { id: 'C008', name: 'Thể Thao', count: 0, status: 'Draft' },
        ];
        ...
        const formatCurrency = (amount) => {
            if (amount === 0 || amount === '0' || amount === null) return 'Miễn Phí';
        ...
        const getStatusStyles = (status) => {
            switch (status) {
                case 'Active': return { text: 'Active', badgeClass: 'bg-success text-white' };
                case 'Draft': return { text: 'Draft', badgeClass: 'bg-warning text-dark' };
                case 'Archived': return { text: 'Archived', badgeClass: 'bg-secondary text-white' };
                case 'Blocked': return { text: 'Blocked', badgeClass: 'bg-danger text-white' };
                default: return { text: status, badgeClass: 'bg-light text-dark' };
            }
        };
        ...
        const synchronizeMetadata = () => {
            ...
            // 2. Đồng bộ số lượng sách cho Thể loại
        ...
        window.saveBook = () => {
            ...
            if (mode === 'add') {
                ...
                if (index !== -1) mockBooksData[index] = { ...mockBooksData[index], ...dataToSave, id: originalId };
