...
try {
// Biến này được truyền từ JSP (Bước 3)
...
const serverCategories = JSON.parse(SERVER_CATEGORIES_JSON);
        // CHUYỂN ĐỔI: Map dữ liệu CSDL sang định dạng Mock/JS đã có
        realCategoryData = serverCategories.map(cat => ({
        id: 'C' + cat.idTheLoai, // Chuyển ID số sang định dạng Cxxx của Mock
                name: cat.tenTheLoai,
                count: 0, // Số lượng sách sẽ được tính sau hoặc lấy từ CSDL (chúng ta sẽ cần cập nhật DAO)
                status: 'Active',
        }));
} else {
...
// --- INITIALIZATION ---
        ...
        // BỔ SUNG: Tải dữ liệu Thể loại thực
